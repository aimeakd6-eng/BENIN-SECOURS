...
export default function SignalerPanneScreen() {
  ...
  const handleOfflineSOS = () => {
    if (!typePanne) {
      setError("Veuillez sélectionner un type de panne avant d'envoyer le SMS");
      return;
    }

    const adminPhone = "+2290100000000"; // Numéro central d'urgence
    const lat = userLocation?.latitude || "Inconnue";
    const lon = userLocation?.longitude || "Inconnue";
    const message = `SOS BENIN SECOURS !
Type: ${typePanne} (${categorieVehicule})
...
Aidez-moi vite !`;
  ...
  return (
    <div className="flex min-h-screen flex-col" style={{ backgroundColor: "var(--color-bg)" }}>
      ...
      {/* Bouton action */}
      {!loading && (
        <footer className="sticky bottom-0 border-t border-gray-200 bg-white p-4 space-y-3">
          <button
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-red-100 bg-white py-3.5 text-xs font-bold text-red-600 transition-all active:scale-[0.98]"
          >
            <MessageCircle className="h-4 w-4" />
            SOS PAR SMS (SANS INTERNET)
          </button>
        </footer>
      )}
    </div>
